...
1. A Qdrant instance to connect to. You can set one up in our installation guide.
2. A running Airflow instance. You can use their Quick Start Guide.
## Installation
...
Open the `Admin-> Connections` section of the Airflow UI. Click the `Create` link to create a new Qdrant connection.
...
```
A `qdrant_client#QdrantClient` instance is available via `@property conn` of the `QdrantHook` instance for use within your Airflow workflows.
```python
