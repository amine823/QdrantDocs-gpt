...
* The maximum number of collections in a cluster is set to 1000. Larger numbers of collections lead to performance degradation. For more information see Multitenancy.
* Strict mode is activated by default for new collections enforcing that all filters being used in retrieve and udpate queries are indexed. This improves performance and reliability. You can disable this individually for each collection. For more information see Strict Mode.
...
Refer to Qdrant Configuration for more details.
...
Async Scorer
Enables async scorer which uses io_uring when rescoring. See Qdrant under the hood: io_uring and Large Scale Search for more details.
