...
When scaling up horizontally, the cloud platform will automatically rebalance all available shards across nodes to ensure that the data is evenly distributed. See Configuring Clusters for more details.
...
To increase the number of shards (reshard up), use the Update collection cluster setup API to initiate the resharding process:
```http
...
The current status of resharding is listed in the collection cluster info which can be fetched with:
```http
