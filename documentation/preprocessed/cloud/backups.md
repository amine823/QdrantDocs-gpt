...
Dashboard at This section assumes that you've already
...
Qdrant Cloud Dashboard. To do so, take these steps:
...
1. Take a snapshot:
 ...
 Then call the snapshot endpoint on the individual hosts. Start with node 0.
 In the response, you'll see the name of the snapshot.
2. Delete and recreate the collection.
3. Recover the snapshot:
 Call the recover endpoint. Set a location which points to the snapshot file (`file:///qdrant/snapshots/{collection_name}/{snapshot_file_name}`) for each host.
