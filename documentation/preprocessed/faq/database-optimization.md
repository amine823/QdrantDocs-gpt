...
Read more about configuring the optimal use of Qdrant.
...
If you want to limit the memory usage of the service, we recommend using limits in Docker or Kubernetes.
### My requests are very slow or time out. What should I do?
There are several possible reasons for that:
 Using filters without payload index -- If you're performing a search with a filter but you don't have a payload index, Qdrant will have to load whole payload data from disk to check the filtering condition. Ensure you have adequately configured payload indexes.
 Usage of on-disk vector storage with slow disks -- If you're using on-disk vector storage, ensure you have fast enough disks. We recommend using local SSDs with at least 50k IOPS. Read more about the influence of the disk speed on the search latency in the article about Memory Consumption.
