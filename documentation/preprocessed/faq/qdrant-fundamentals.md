...
Qdrant natively supports multiple vectors per data point, allowing different embeddings from various providers to coexist within the same collection.
...
If you need an exact number, you can use the count API.
...
You are likely looking for the scroll method. It allows you to retrieve the records based on filters or even iterate over all the records in the collection.
...
Read about our recommendations in the bulk upload tutorial.
