...
Using Docker/WSL on Windows with mounts is known to have file system problems causing data loss. See troubleshooting.
### Networking
...
```bash
helm repo add qdrant the linked resource
...
configs:
  qdrant_config:
    content: |
      log_level: INFO
```
Providing the inline content in the configs top-level element requires Docker Compose v2.23.1 or above. This functionality is supported starting Docker Engine v25.0.0 and Docker Desktop v4.26.0 onwards.
### From source
