...
3. TLS: enable encrypted traffic everywhere using TLS.
## Authentication
...
docker run -p 6333:6333 \
    ...
    qdrant/qdrant
```
TLS must be used to prevent leaking the API key over an unencrypted connection.
...
```bash
curl \
  -X GET the local Qdrant dashboard \
...
For more complex cases, Qdrant supports granular access control with JSON Web Tokens (JWT).
...
connections.
Connections are unencrypted by default. This allows sniffing and MitM attacks.
...
```bash
curl -X GET the local Qdrant dashboard
```
```python
from qdrant_client import QdrantClient
client = QdrantClient(
...
```typescript
import { QdrantClient } from "@qdrant/js-client-rest";
const client = new QdrantClient({ url: "the linked resource port: 6333 });
```
```rust
use qdrant_client::Qdrant;
let client = Qdrant::from_url("the linked resource
...
* Block Qdrant's external network access. This can help mitigate server side request forgery attacks, like via the snapshot recovery API. Single-node Qdrant clusters do not require any outbound network access. Multi-node Qdrant clusters only need the ability to connect to other Qdrant nodes via TCP ports 6333, 6334, and 6335.
 You can use `docker network create --internal ` and use that network when running `docker run --network `.
