...
2D latent space. Both texts are examples of English pangrams.
 generated the images with pangrams used as input prompts.*
## Sample dataset
You will be using COCO, a large-scale object detection, segmentation, and captioning dataset. It provides
various splits, 330,000 images in total. For demonstration purposes, this tutorials uses the
2017 validation split that contains 5000 images from different
categories with total size about 19GB.
```terminal
wget the linked resource
```
## Prerequisites
There is no need to curate your datasets and train the models. Aleph Alpha, already has multimodality and multilinguality already built-in. There is an official Python client that simplifies the integration.
...
1. You need to have a Qdrant instance running. If you want to launch it locally,
 Docker is the fastest way to do that.
2. You need to have a registered Aleph Alpha account.
3. Upon registration, create an API key (see: API Tokens).
