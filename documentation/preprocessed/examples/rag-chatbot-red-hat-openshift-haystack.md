...
```shell
export INFERENCE_ENDPOINT_URL="the linked resource
```
```python
import os
os.environ["INFERENCE_ENDPOINT_URL"] = "the linked resource
```
...
### Indexing pipeline
Haystack 2.x comes packed with a lot of useful components, from data fetching, through
...
hayhooks run
```
The command should start the service on the default port, so you can access it at ` The pipeline
...
the local API documentation interface, and test the newly created endpoint.
...
environment, so you can easily move it to your own OpenShift infrastructure.
 If you are just getting started and need more guidance on Qdrant, read the quickstart or try out our beginner tutorial.
