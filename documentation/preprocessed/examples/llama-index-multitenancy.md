...
called multitenancy. Our guide on the Separate Partitions describes
how to set it up in general, but if you use LlamaIndex as a
...
to set up the Qdrant payload index, so the search
is more efficient.
```python
...
graph is not built globally. This is also done due to performance reasons.
