...
an ingestion pipeline and then a Retrieval Augmented Generation application that will use the data.
 ...
 users as an incrementally updated Excel sheet
 ...
 Ingestion pipeline: Airbyte, loading the data into Qdrant
 Large Language Model: Cohere Command-R
 RAG: Cohere RAG using our knowledge base
 through a custom connector
All the selected components are compatible with the AWS infrastructure. Thanks to Cohere models' availability, you can build a fully private customer support system completely isolates data within your infrastructure. Also, if you have AWS credits, you can now use them without spending additional money on the models or
...
a ticketing system, such as Zendesk Support,
Freshdesk, or maybe integrate it with a shared inbox. However, in case of
...
sheet available over HTTP/FTP that Airbyte can access and load into
Qdrant in an incremental manner.
### Cohere Qdrant Connector for RAG
Cohere RAG relies on connectors which brings additional context to the model.
...
guide on how to deploy the models in that way, so you can
...
```shell
export QDRANT_URL="the linked resource
export QDRANT_API_KEY="your-api-key"
```
```python
import os
os.environ["QDRANT_URL"] = "the linked resource
...
databases. You can install it on your infrastructure and use it to load the data into Qdrant. The installation process is described in the official documentation.
...
use the following connectors:
 Source: File to load the data from an Excel sheet
 Destination: Qdrant to load the data into Qdrant
...
related tutorial is a perfect fit for this task. The only difference is that you
...
(EKS), or by launching an EC2 compute instance. This step
...
```shell
export RAG_CONNECTOR_URL="the linked resource
```
```python
os.environ["RAG_CONNECTOR_URL"] = "the linked resource
```
## Customer interface
