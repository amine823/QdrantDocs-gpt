...
All storage volumes are encrypted at rest. Premium customers can also bring their own encryption keys for storage volumes.
Data in transit is protected with Transport-Layer-Security (TLS). It is possible to restrict the IP ranges that are allowed to access a cluster.
Infrastructure access is restricted and audited according to the policies described in our Trust Center. All infrastructure components are regularly patched and updated to ensure up to date security.
Access to Qdrant Cloud accounts can be configured with granular Role-Based Access Control. Premium customers can also enable Single Sign-On (SSO) with their identity provider.
API keys can be configured with granular access controls and rotated at any time. API Keys are never stored in plaintext.
...
Please refer to our Hybrid Cloud documentation for more details.
...
Please refer to our Private Cloud documentation for more details. As Private Cloud is completely isolated from the Qdrant Cloud Console, we encourage you to assess your security requirements against Hybrid Cloud before considering Private Cloud for the best experience.
...
```
All of our container images are scanned for vulnerabilities using Trivy.
All of our container images and helm charts are signed using Cosign. You can verify the signature of an image with `cosign`:
```bash
cosign verify registry.cloud.qdrant.io/qdrant/operator:latest --certificate-oidc-issuer=the linked resource --certificate-identity-regexp='the linked resource
```
## Terms of Service and Data Processing Agreement
By using Qdrant Cloud, you agree to our Terms of Service and Privacy Policy. For customers subject to GDPR, we also offer a Data Processing Agreement (DPA) that outlines our commitments regarding data protection and privacy.
