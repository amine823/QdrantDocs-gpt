...
on an example of the real-world dataset LAION-400M.
...
```
In practice the size of index is a bit smaller due to the compression we implemented in Qdrant v1.13.0, but it is still a good estimation.
...
IdTracker {
    ...
    internal_to_external: Vec, // 400M x 16 = 6.4Gb
    ...
    external_to_internal: Vec, // 400M x (8 + 4) = 4.5Gb
}
```
In the v1.13.5 we introduced a significant optimization to reduce the memory usage of `IdTracker` by approximately 2 times.
...
we provide a ready-to-use ground truth file and the script to generate it (requires 512Gb RAM machine and about 20 hours of execution time).
...
You can find the complete code of the search process in the eval.py
## Performance tweak
One important performance tweak we found useful for this dataset is to enable Async IO in Qdrant.
...
You can find the full code of the search process in the eval.py
