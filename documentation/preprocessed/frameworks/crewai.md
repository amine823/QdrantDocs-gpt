...
You can learn to set up a CrewAI project here. Let's assume the project was name `mycrew`.
...
class QdrantStorage(RAGStorage):
    ...
    def _initialize_app(self):
        ...
        # and choose from the [supported embedders](the linked resource
...
```
The `add` AND `query` methods use FastEmbed to vectorize data. You can however customize it if required.
### Instantiate your crew
You can learn about setting up agents and tasks for your crew here. We can update the instantiation of `Crew` to use our storage mechanism.
> src/mycrew/crew.py
```python
