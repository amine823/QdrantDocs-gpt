...
Massive Text Embedding Benchmark (MTEB). The evaluation process itself is pretty
...
search quality.
### Quality metrics
There are various ways of how quantify the quality of semantic search. Some of them, such as Precision@k#Precision_at_k),
are based on the number of relevant documents in the top-k search results. Others, such as Mean Reciprocal Rank (MRR),
take into account the position of the first relevant document in the search results. DCG and NDCG
...
parameterizable and tunable, when required. There are some other ANN algorithms available out there, such as IVF*,
but they usually perform worse than HNSW in terms of quality and performance.
