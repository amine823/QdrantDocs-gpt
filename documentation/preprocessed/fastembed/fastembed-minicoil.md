...
"miniCOIL: on the road to Usable Sparse Neural Retreival" article or, in a recorded talk "miniCOIL: Sparse Neural Retrieval Done Right".
...
```
Then, initialize the Qdrant client. You could use for experiments a free cluster in Qdrant Cloud or run a local Qdrant instance via Docker.
...
As miniCOIL was designed with Qdrant's ability to calculate the keywords Inverse Document Frequency (IDF) in mind, we need to configure miniCOIL sparse vectors with IDF modifier.
