...
cluster in Qdrant Cloud.
```python
from qdrant_client import QdrantClient, models
client = QdrantClient("the local Qdrant dashboard")
```
Create a collection that stores both MUVERA embeddings and the original
multi-vector representations using named vectors.
```python
...
queries through the `prefetch` parameter, which lets us
